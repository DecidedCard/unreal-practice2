...
- 시작 상태를 설정할 필요 없이 왼쪽에서부터 깊이 우선 탐색을 시작한다.
- 행동을 중심으로 설계한다.
...
- 헤드업디스플레이는 플레이어 컨트롤러에 의해 제작되고 관리되는 UI 객체
